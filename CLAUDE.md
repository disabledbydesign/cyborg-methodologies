...
**Voice-check profile copies out of sync (flagged 2026-06-16, S29 of propositional-memory-architecture)**: Claude's own voice profile `claude.json` lives and is edited at `~/.claude/skills/voice-check/profiles/`, but the source `voice-check/profiles/` *here* holds only `base.json` — the two voice-check locations have drifted (the `~/.claude/skills/voice-check/` copy is a real directory, NOT a symlink to this repo, unlike critic-swarm/printpress/workshop). Consequence: edits to `claude.json` (e.g. the S29 addition of the `descriptions_before_labels` post-draft legibility check) are **not version-controlled in this repo**. Cleanup: reconcile the two dirs — either symlink `~/.claude/skills/voice-check` → this repo (matching the other skills) and bring `claude.json` under version control here, or document the split deliberately. Low-risk but latent.

**Voice-check pre-draft enforcement (parked 2026-05-19, surfaced via UCSC CRES Lecturer session)**: Agents consistently under-execute the pre-draft voice-check work — read profile as data-to-extract rather than prose-to-absorb; treat voice-final priors as comparators rather than source prose; produce agent-shaped output that passes the post-draft contamination linter but doesn't sound like June. Quick fixes landed 2026-05-19 in printpress SKILL.md § Stage 2 + genre_configs/academic_position.md § Voice-check during drafting (explicit "Read tool not python extraction"; "priors are SOURCE, not comparators"; "begin Stage 2 by listing which prior passages adapt to which sections"). Expected: failure rate drops but not to zero. Three structural fixes remain:

1. **Voice profile in markdown, not JSON.** JSON triggers parse-and-extract pattern. Markdown invites reading-as-prose. Translate `profiles/*.json` to a markdown-with-frontmatter format (writing_check.py still needs structured access to thresholds — keep JSON as machine-readable sidecar, or use YAML frontmatter + markdown body). The qualitative content — `style_notes`, `qualitative.checks[].instruction`, anti-pattern lists, writing samples — belongs in prose where it reads as guidance.

2. **Pre-draft verification step.** Nothing currently verifies that the agent actually internalized the voice content before writing. Only output gets checked. Add a forcing-function checkpoint: before Stage 2 drafting can begin, the agent must write back (in DRAFT_PLAN.md or in chat) a) which specific prior passages will be adapted for each section (with file paths and paragraph identifiers), b) which qualitative checks/anti-patterns from the profile were read and what they mean for this draft, c) at least one paragraph of prior prose marked verbatim that will be adapted. Without this checkpoint, pre-draft slippage is structurally invisible.

3. **Success-signal asymmetry between pre-draft and post-draft.** Post-check produces a concrete report with green/red flags — clear "done" signal. Pre-draft is amorphous. Agents gravitate toward concrete success signals and under-execute amorphous ones. Fix #2 above partially addresses this (the verification step produces a concrete artifact), but a stronger fix would give pre-draft work its own report-shaped output (e.g., "voice modeling complete: N prior passages identified, M pre_draft checks read, draft can begin").

Items 1–3 are interdependent: markdown profile (#1) makes the content readable; verification step (#2) enforces reading; success-signal change (#3) makes the enforcement feel like progress rather than ceremony. Best done together as a coherent voice-check-pre-draft architecture revision, not piecemeal.
