...
@pytest.fixture
def default_profile():
    """Load the base.json profile from disk (formerly default.json).

    The 8-genre schema has moved to user profiles. base.json contains
    universal norms only (no genres). This fixture loads base.json for
...
# ---------------------------------------------------------------------------
# base.json genre schema validation
#
# Genres moved from default.json (now base.json) to user profiles in the
# three-layer merge architecture. base.json ships with no genres — users
# define their own via calibration or the agent genre-creation walkthrough.
# These tests validate the genre schema when a user profile provides genres.
# ---------------------------------------------------------------------------

@pytest.mark.skip(reason=(
    "Genre schema tests require a user profile with genres. "
    "Genres moved from base.json to user profiles in the three-layer architecture. "
    "base.json ships with no genres. See TestApplyProfileGenre for genre-override tests."
))
...
#
# These tests validated genre differentiation when genres were in default.json.
# Genres now live in user profiles. Skip until a user profile fixture is added.
# ---------------------------------------------------------------------------

@pytest.mark.skip(reason=(
    "Genre register tests require a user profile with the 8-genre schema. "
    "Genres moved from base.json to user profiles in the three-layer architecture."
))
...
class TestCalibrationGenreOutput:
    ...
    def test_calibrated_profile_has_base_reference(self, tmp_path):
        """Profiles generated by --calibrate should reference base.json if it exists."""
        ...
        # In the three-layer architecture, calibration produces a sparse profile
        # that references base.json (if available). Genres are user-defined, not
        # generated by calibration — the agent walks users through genre creation.
        assert "base" in profile or "thresholds" in profile  # sparse or standalone

    def test_calibrated_patterns_section_is_empty(self, tmp_path):
        """Calibration produces an empty patterns dict — user adds personal anti-patterns."""
        samples = tmp_path / "samples"
        samples.mkdir()
        (samples / "s1.md").write_text(
            "I built this tool because the institution failed its students. "
        ...
        profile = json.loads(output.read_text(encoding="utf-8"))
        # Patterns section should be empty — user adds personal anti-patterns.
        # Base patterns are loaded at runtime via base.json merge.
